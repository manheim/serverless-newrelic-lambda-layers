...
export default class NewRelicLambdaLayerPlugin {
  ...
  private async getLayerArn(runtime: string, region: string) {
    return util
      .promisify(request)(
        {
          uri: `https://${region}.nr-layers.iopipe.com/get-layers?CompatibleRuntime=${runtime}`
        },
        (err, response) => JSON.parse(response.body)
      )
      .then(body => {
        return _.get(body, "Layers[0].LatestMatchingVersion.LayerVersionArn");
      });
  }
